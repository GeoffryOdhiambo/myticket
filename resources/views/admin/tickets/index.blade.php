                ...
                @foreach($tickets as $ticket)
                    ...
                    <div class="p-4">
                        <div class="flex items-start justify-between gap-3">
                            <div class="min-w-0">
                                <p class="truncate text-sm font-semibold text-neutral-900">{{ $ticket->ticket_number }}</p>
                                <p class="mt-0.5 truncate text-xs text-neutral-500">{{ $order->event->name }}</p>
                            </div>
                            <x-badge :color="$ticket->checked_in ? 'success' : 'neutral'">{{ $ticket->checked_in ? 'Checked In' : 'Not Checked In' }}</x-badge>
                        </div>

                        <p class="mt-2 truncate text-xs text-neutral-400">{{ $order->event->organizer->business_name }} · {{ $order->customer_name }}</p>

                        <div class="mt-2 flex items-center justify-between text-xs">
                            <span class="text-neutral-500">{{ $ticket->orderItem->ticketType->name }} · {{ $settings->formatPrice($ticket->orderItem->unit_price) }}</span>
                            <span class="text-neutral-400">{{ $ticket->created_at->format('d M Y') }}</span>
                        </div>
                    </div>
                @endforeach
            </div>
