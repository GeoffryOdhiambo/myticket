            ...
            <div class="overflow-x-auto">
                <table class="w-full text-left text-sm">
                    <thead class="border-b border-neutral-100 bg-neutral-50 text-xs font-semibold uppercase tracking-wide text-neutral-500">
                        <tr>
                            <th class="px-3 py-2.5 sm:px-5 sm:py-3">Ticket No.</th>
                            ...
                            <th class="px-3 py-2.5 sm:px-5 sm:py-3">Type</th>
                    ...
                    <tbody class="divide-y divide-neutral-100">
                        @foreach($tickets as $ticket)
                            ...
                            @endphp
                            <tr>
                                <td class="px-3 py-2.5 sm:px-5 sm:py-3.5 font-semibold text-neutral-900">{{ $ticket->ticket_number }}</td>
                                <td class="px-3 py-2.5 sm:px-5 sm:py-3.5 text-neutral-600">{{ $order->event->name }}</td>
                                <td class="px-3 py-2.5 sm:px-5 sm:py-3.5 text-neutral-500">{{ $order->event->organizer->business_name }}</td>
                                <td class="px-3 py-2.5 sm:px-5 sm:py-3.5 text-neutral-500">{{ $order->customer_name }}</td>
                                <td class="px-3 py-2.5 sm:px-5 sm:py-3.5 text-neutral-600">{{ $ticket->orderItem->ticketType->name }}</td>
                                <td class="px-3 py-2.5 sm:px-5 sm:py-3.5 text-neutral-600">{{ $settings->formatPrice($ticket->orderItem->unit_price) }}</td>
                                ...
                                <td class="px-3 py-2.5 sm:px-5 sm:py-3.5 text-neutral-500">{{ $ticket->created_at->format('d M Y') }}</td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
