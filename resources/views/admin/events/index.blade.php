        ...
        @else
            ...
            <div class="divide-y divide-neutral-100 lg:hidden">
                @foreach($events as $event)
                    <a href="{{ route('admin.events.show', $event) }}" class="block p-4 active:bg-neutral-50">
                        <div class="flex items-start justify-between gap-3">
                            <div class="min-w-0">
                                <p class="truncate text-sm font-semibold text-neutral-900">{{ $event->name }}</p>
                                <p class="mt-0.5 truncate text-xs text-neutral-500">{{ $event->organizer->business_name }} · {{ $event->event_date->format('d M Y') }}</p>
                            </div>
                        ...
                        <div class="mt-2.5 flex items-center justify-between text-xs">
                            <span class="text-neutral-500">{{ $event->tickets_sold }} sold</span>
                            <span class="font-semibold text-neutral-900">{{ $settings->formatPrice($event->revenue) }}</span>
                        </div>
                    </a>
                @endforeach
            </div>
