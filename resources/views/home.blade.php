...
<x-layouts.app>
    ...
    <section class="relative overflow-hidden bg-neutral-950">
        ...
        <div class="relative mx-auto max-w-5xl px-4 py-28 text-center sm:px-6 sm:py-36 lg:px-8">
            ...
            <p class="mx-auto mt-6 max-w-xl text-base text-neutral-200 sm:text-lg">
            ...
            <form action="{{ route('events.index') }}" method="GET" class="mx-auto mt-8 flex max-w-lg flex-col gap-3 sm:flex-row">
    ...
    <section class="mx-auto max-w-7xl px-4 py-20 sm:px-6 lg:px-8">
        <div class="flex items-end justify-between animate-in">
            <div>
                <p class="text-sm font-semibold uppercase tracking-wide text-brand">Don't Miss Out</p>
                <h2 class="mt-1 text-2xl font-extrabold text-neutral-900 sm:text-3xl">Featured Events</h2>
            </div>
    ...
    <section class="bg-neutral-50 py-20">
        <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <div class="text-center animate-in">
                <p class="text-sm font-semibold uppercase tracking-wide text-brand">Browse by Category</p>
                <h2 class="mt-1 text-2xl font-extrabold text-neutral-900 sm:text-3xl">Find Your Next Experience</h2>
            </div>
            ...
            <div class="mt-10 grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-6">
                @foreach($categories as $category)
                    <a
                        ...
                        class="animate-in flex flex-col items-center gap-3 rounded-2xl border border-neutral-100 bg-white p-6 text-center shadow-sm transition-shadow hover:shadow-md"
                        style="transition-delay: {{ $loop->index * 0.05 }}s"
                    >
                        <span class="flex h-12 w-12 items-center justify-center rounded-full bg-brand-light text-brand-dark">
                            <x-dynamic-component :component="'heroicon-o-' . $category->icon" class="h-6 w-6" />
                        </span>
                        <span class="text-sm font-semibold text-neutral-800">{{ $category->name }}</span>
                    </a>
                @endforeach
            </div>
        </div>
    ...
    <section class="mx-auto max-w-7xl px-4 py-20 sm:px-6 lg:px-8">
        <div class="text-center animate-in">
            <p class="text-sm font-semibold uppercase tracking-wide text-brand">Simple &amp; Fast</p>
            <h2 class="mt-1 text-2xl font-extrabold text-neutral-900 sm:text-3xl">How Tiko Works</h2>
        </div>
        ...
        <div class="mt-10 grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
            @foreach([
                ...
                <div class="animate-in text-center" style="transition-delay: {{ $loop->index * 0.08 }}s">
                    <span class="mx-auto flex h-14 w-14 items-center justify-center rounded-2xl bg-neutral-900 text-white">
                        <x-dynamic-component :component="'heroicon-o-' . $step['icon']" class="h-6 w-6" />
                    </span>
                    <p class="mt-4 text-xs font-semibold uppercase tracking-wide text-brand">Step {{ $loop->iteration }}</p>
                    <h3 class="mt-1 text-base font-bold text-neutral-900">{{ $step['title'] }}</h3>
                    <p class="mt-2 text-sm text-neutral-500">{{ $step['text'] }}</p>
                </div>
            @endforeach
        </div>

        <div class="mt-14 grid grid-cols-3 gap-4 border-t border-neutral-100 pt-10 text-center">
            <div class="animate-in">
                <p class="text-3xl font-extrabold text-neutral-900 sm:text-4xl"><span data-counter-target="{{ $stats['events'] }}">0</span>+</p>
                <p class="mt-1 text-sm text-neutral-500">Events Hosted</p>
            </div>
            <div class="animate-in" style="transition-delay: 0.08s">
                <p class="text-3xl font-extrabold text-neutral-900 sm:text-4xl"><span data-counter-target="{{ $stats['organizers'] }}">0</span>+</p>
                <p class="mt-1 text-sm text-neutral-500">Organizers</p>
            </div>
            <div class="animate-in" style="transition-delay: 0.16s">
                <p class="text-3xl font-extrabold text-neutral-900 sm:text-4xl"><span data-counter-target="{{ $stats['tickets'] }}">0</span>+</p>
                <p class="mt-1 text-sm text-neutral-500">Tickets Sold</p>
            </div>
        </div>
    </section>
    ...
    <section id="for-organizers" class="bg-neutral-900">
        <div class="mx-auto max-w-7xl px-4 py-20 sm:px-6 lg:px-8">
            <div class="grid items-center gap-10 lg:grid-cols-2">
                <div class="animate-in">
                    <p class="text-sm font-semibold uppercase tracking-wide text-brand">Hosting an Event?</p>
                    <h2 class="mt-2 text-2xl font-extrabold text-white sm:text-3xl">Sell your tickets online with Tiko and make entry management simple.</h2>
